...
public class BeanUtil {
    ...
    public static <T, S> Set<T> convert(Set<S> sources, Class<T> clazz) {
        return Optional.ofNullable(sources).map(each -> {
            Set<T> targetSize = new HashSet<>(each.size());
     ...
     * @return 转换后对象集合
     */
    public static <T, S> List<T> convert(S[] sources, Class<T> clazz) {
        return Optional.ofNullable(sources).map(each -> {
            List<T> targetList = new ArrayList<>(sources.length);
            for (S source : sources) {
                targetList.add(BEAN_MAPPER_BUILDER.map(source, clazz));
            }
            return targetList;
        }).orElse(null);
    }
