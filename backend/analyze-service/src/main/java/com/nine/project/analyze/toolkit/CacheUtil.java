...
public class CacheUtil<T> {
     ...
     */
    public List<GithubUserScoreRankRespDTO> getGithubUserScoreRankFromCache(List<String> nations, List<String> types,Integer size) {
        String key;
        if (types!= null &&!types.isEmpty()) {
            key = String.format(SCORE_RANK_NATION_TYPE_KEY, nations, types, size);
        } else {
            key = String.format(SCORE_RANK_NATION_KEY, nations, size);
        }
     ...
     * @param types         用户类型列表，如果为 null 或空列表则不根据类型筛选。
     */
    public void setGithubUserScoreRankToCache(List<GithubUserScoreRankRespDTO> scoreRankList, List<String> nations, List<String> types, Integer size) {
        String key;
        if (types!= null &&!types.isEmpty()) {
            key = String.format(SCORE_RANK_NATION_TYPE_KEY, nations, types, size);
        } else {
