...
const CardWithScrollableTable = <T,>({ title, columns, data, dataCount, renderRow, error, reload }: DataTableProps<T>) => {
    ...
    const handleScroll = throttleWithDeepClone((event: React.UIEvent<HTMLDivElement>) => {
        ...
        if (event.currentTarget.scrollTop > 0) {
            ...
            headerRef.current?.classList.remove('my-4', 'text-lg', 'font-bold');
            headerRef.current?.classList.add('text-base');
        } else {
            headerRef.current?.classList.remove('text-base');
            headerRef.current?.classList.add('my-4', 'text-lg', 'font-bold');
        }
    }, 200)
            ...
            <div className="overflow-y-auto max-h-max flex-grow scrollbar" onScroll={handleScroll}>
                {data ? (
                    <table className="w-full">
                        <thead>
                            <tr className="mb-6 sticky top-0 font-normal bg-white pb-3">
