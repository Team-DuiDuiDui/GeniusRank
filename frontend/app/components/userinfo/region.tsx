...
const UserNation = ({ nationISO, nationLocale, warning, loading, disable }: NationCardProps) => {
    ...
    return (
        <CardWithNoShrink containerClass={`flex-shrink-0 flex-grow-0 overflow-hidden `} containerClassDelete={["p-8"]}>
            ...
            <div className={`h-full w-full flex items-center justify-center p-4 `} style={{ aspectRatio: "4/3" }}>
                {disable ? <></> : <div className="absolute w-6 h-6 top-3 right-3 bg-white/90 backdrop-blur-md rounded-full shadow-md">
                ...
                {nationISO !== "CN" &&
                    <div className=" shadow-md rounded-full border h-full bg-white/70 backdrop-blur-md aspect-square flex flex-col items-center justify-center" style={{ borderColor: "#39c5bb" }}>
                        ...
                        <>
                            <InfoIcon color="red" /> 
                            <div className=" text-base drop-shadow-md font-bold text-red-600">登录后查看猜测</div> 
                            <div className=" text-base drop-shadow-md font-bold text-red-600">的国籍相关信息</div> 
                        </>
                        : 
                        <>
