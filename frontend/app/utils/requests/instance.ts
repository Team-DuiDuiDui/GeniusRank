...
/**
 * 能在速率限制后感觉返回标头选择指数退避或根据标头重试请求，同时显示 toast
...
export const createInstanceForGithubClient = (token?: string): AxiosInstanceForGithub => {
    ...
    interceptor.interceptors.response.use(null, async (err) => {
        ...
        if (err.response?.headers['x-ratelimit-remaining'] == 0) {
            ...
            toast.loading(
                `${i18next.t('user.err.reloading_after')} ${Math.round((resetTime - localTime) / 1000)} ${i18next.t(
                    'user.err.reloading_after_2'
                )}`,
                {
                    duration: resetTime - localTime,
                }
            );
