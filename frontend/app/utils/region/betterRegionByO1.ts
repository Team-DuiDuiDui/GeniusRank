...
function calculateNationPrediction(fansList: User[], followersList: User[]) {
    // 定义 User 接口
    ...
    function calculateSocialInfluenceWeight(count: number, maxCount: number): number {
        const adjustedCount = Math.min(count, maxCount);
    ...
    // 获取标准化后的地理位置计数
    function getLocationCounts(list: User[]): Record<string, number> {
        return list.reduce((counts, user) => {
            const standardizedLocation = standardizeLocation(user.location);
            if (standardizedLocation) {
                counts[standardizedLocation] = (counts[standardizedLocation] || 0) + 1;
            }
            return counts;
        }, {} as Record<string, number>);
    }

    // 计算先验概率（假设有一个全局的地理分布数据 globalLocationDistribution）
    ...
    function calculatePosteriorProbabilities(
        ...
        const maxCount = 10000; // 设定粉丝或关注数量的最大阈值
        ...
        list.forEach((user, index) => {
            ...
            const influenceWeight = calculateSocialInfluenceWeight(
                isFollowerList ? user.followers : user.following,
                maxCount
            );
            const standardizedLocation = standardizeLocation(user.location);
            const priorProb = standardizedLocation ? getPriorProbability(standardizedLocation, globalLocationDistribution) : 0;

            const weight = timeWeight * influenceWeight * priorProb;
        ...
        const totalWeight = Object.values(locationWeights).reduce((sum, weight) => sum + weight, 0);
    ...
    // 全球地理分布数据，数据来源 http://47.121.222.224:30013/#/ 和 github 2021 年公开报表
    ...
    const followerResults = processList(followersList, true);
